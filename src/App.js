...
export default function App() {
  ...
  function abrirModalEditar(rastreador) {
    ...
    setForm({
      ...
      responsavel: rastreador.responsavel,
    })
    setModalAberto(true)
  }
    ...
    if (editando) {
      ...
      if (!error) {
        setRastreadores(prev =>
          prev.map(r => (r.id === editando ? { ...r, ...form } : r))
        )
      }
    } else {
      const { data, error } = await supabase
        ...
        .select()
      if (!error) {
        setRastreadores(prev => [...prev, data[0]])
      }
    }
  ...
  async function deletar(id) {
    ...
    const { error } = await supabase
      ...
      .eq('id', id)
    if (!error) {
      setRastreadores(prev => prev.filter(r => r.id !== id))
    }
    setDeletando(null)
  }
        ...
        {loading ? (
          ...
          <div className="flex flex-col gap-3 mt-1">
            {rastreadores.map(r => (
              <div
                key={r.id}
                className="bg-slate-800 border border-slate-700 rounded-2xl p-4 flex items-center justify-between gap-3 shadow-sm"
              >
                ...
                <div className="flex items-center gap-1 shrink-0">
                  <button
                    ...
                    className="p-2 text-slate-400 hover:text-blue-400 hover:bg-slate-700 rounded-xl transition-colors"
                  >
                    <Pencil size={17} />
                  </button>
                  <button
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-2xl w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between p-5 border-b border-slate-700">
              <h2 className="text-base font-bold">
                {editando ? 'Editar Rastreador' : 'Novo Rastreador'}
              </h2>
              <button
                onClick={fecharModal}
            ...
            <div className="p-5 flex flex-col gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-900 border border-slate-600 focus:border-blue-500 text-white placeholder-slate-500 rounded-xl px-4 py-3 text-sm outline-none transition-colors font-mono tracking-wider uppercase"
                />
              </div>
              <div>
                <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider block mb-1.5">
            ...
            <div className="flex gap-3 p-5 pt-0">
              <button
                onClick={fecharModal}
                className="flex-1 bg-slate-700 hover:bg-slate-600 text-white font-semibold py-3 rounded-xl transition-colors text-sm"
              >
                Cancelar
              </button>
              <button
                onClick={salvar}
                disabled={salvando || !form.nome || !form.placa || !form.responsavel}
                className="flex-1 bg-blue-500 hover:bg-blue-600 disabled:bg-slate-600 disabled:text-slate-400 text-white font-semibold py-3 rounded-xl transition-colors text-sm flex items-center justify-center gap-2"
              >
                {salvando ? (
